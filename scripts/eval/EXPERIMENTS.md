...
## Queued (durable record — audit 2026-07-22; each needs its own pre-registration before running)

| Queued item | Earliest / target date | Anchor |
|---|---|---|
| Insider holdout confirmation (E003, `insider_net_z_lag2`) | **~2026-08-28** (≥40 post-outage trading days) | Booked in `HOLDOUT.md` — the only pending holdout use |
| Latency re-audit (E002 middle-band verdict) | **~late Aug 2026** (≥30 days clean accumulation from 07-22) | E002 row decision |
| Adaptive EMA (E005) | when picked up | E005 row (queued) |
| Rank/winsorized insider_net_z variant (heavy-tail fix) | when picked up; sensible to fold into post-holdout insider work | E003 annex cautions |
| short_vol_z re-examination | **~Oct 2026** (≥3 months FINRA history; series began 2026-05-07) | E003 row (rejected-on-thin-sample note) |
| Options-flow evaluation | **blocked** — no options collection exists (see audit: pipeline/sources/options.py was never built); requires commissioning collection first (data-source decision: Finnhub free tier 403s options endpoints), then ≥1 month accumulation | this section (first durable record) |
